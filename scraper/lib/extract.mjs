...
/**
 * 判斷萃取到的央文號是否為真正的中央（衛福部長期照顧司）函釋。
 * 依網站定調：以「衛部顧字第」開頭者為準，排除地方局處自行發文
 * （如桃社老字、府社障字、桃建企字等）以及衛福部其他業務單位文號
 * （衛部醫字、衛部照字、疾管感字、勞動發管字等），只留長照司本身發文。
 */
export function isCentralCircular(centralDocNumber) {
  return !!centralDocNumber && centralDocNumber.startsWith("衛部顧字第");
}
...
export function toRecord(raw, idPrefix, idx) {
  ...
  // 網站定調以中央（衛福部長期照顧司）函釋為準，地方局處自行發文不予收錄
  if (!isCentralCircular(centralDocNumber)) return null;

  // 日期優先順序：優先採用嵌在原始標題中的「央發文日期」（如「115年8月10日衛部顧字第...」
  // 這種格式，代表衛福部實際發文日），只有標題本身沒有日期時才退回使用抓取來源提供的
  // 日期（例如地方政府列表頁的「最後異動時間」，那只是轉知/上架日期，不是央文發文日）
  const date = extractDate(raw.rawTitle) || raw.date || "2020-01-01";
